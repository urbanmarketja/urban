-- Urban Market JA MySQL schema for managed database hosts such as Aiven.
-- Run this inside the Aiven database that was created for the site.
-- Do not run local-only CREATE DATABASE, CREATE USER, or GRANT statements on Aiven.
...
CREATE TABLE IF NOT EXISTS stores (
  ...
  latitude DECIMAL(10, 7),
  longitude DECIMAL(10, 7),
  theme_key VARCHAR(40) NOT NULL DEFAULT 'street',
  theme_primary_color CHAR(7),
  theme_accent_color CHAR(7),
  theme_background_color CHAR(7),
...
CREATE TABLE IF NOT EXISTS cart_items (
  ...
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  UNIQUE KEY uq_cart_items_cart_product_customization (cart_id, product_id, customization_signature),
...
ALTER TABLE cart_items
  ADD COLUMN customization_signature VARCHAR(80) NOT NULL DEFAULT '' AFTER unit_price_jmd;

SET @cart_item_old_unique_exists = (
  SELECT COUNT(*)
...
SET @sql = IF(
  @cart_item_old_unique_exists = 1,
  'ALTER TABLE cart_items DROP INDEX uq_cart_items_cart_product',
  'SELECT "uq_cart_items_cart_product already migrated" AS message'
);
...
SET @cart_item_new_unique_exists = (
  SELECT COUNT(*)
  FROM information_schema.statistics
  WHERE table_schema = DATABASE()
    AND table_name = 'cart_items'
...
ALTER TABLE vendor_wallet_ledger
  ADD CONSTRAINT fk_vendor_wallet_ledger_checkout_request
  FOREIGN KEY (checkout_request_id) REFERENCES vendor_checkout_requests(id) ON DELETE SET NULL;
...
ALTER TABLE vendor_wallet_ledger
  ADD CONSTRAINT fk_vendor_wallet_ledger_service_booking
...
ALTER TABLE vendor_wallet_ledger
  ADD CONSTRAINT fk_vendor_wallet_ledger_payment_session
...
CREATE INDEX idx_product_customization_templates_product_id ON product_customization_templates(product_id);
CREATE INDEX idx_product_customization_surfaces_template_id ON product_customization_surfaces(template_id);
CREATE INDEX idx_product_customization_fields_template_id ON product_customization_fields(template_id);
CREATE INDEX idx_product_customization_field_options_field_id ON product_customization_field_options(field_id);
CREATE INDEX idx_product_customization_placements_field_id ON product_customization_placements(field_id);
CREATE INDEX idx_product_customization_placements_surface_id ON product_customization_placements(surface_id);
...
CREATE INDEX idx_cart_item_customizations_cart_product ON cart_item_customizations(cart_id, product_id, customization_signature);
CREATE INDEX idx_cart_item_customization_previews_cart_product ON cart_item_customization_previews(cart_id, product_id, customization_signature);
...
CREATE INDEX idx_order_item_customization_previews_order_item_id ON order_item_customization_previews(order_item_id);
